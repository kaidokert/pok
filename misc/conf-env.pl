...
# Add -m32 only for native x86 builds (not cross-compilation)
# Skip for ARM and cross-compilation toolchains (i386-elf, etc.)
if (defined $ENV{'ARCH'} && $ENV{'ARCH'} eq "x86" &&
    (!defined $ENV{'CC'} || $ENV{'CC'} !~ /i386-elf|cross/)) {
    push @cflags, "-m32";
}
...
$tools_arch{"arm"} =
{
 ...
 "QEMU"        =>  ["qemu-system-arm", "arm-softmmu"],
 };
...
# {{ Functions definitions
   ...
   sub check_sys ()
   {
      ...
      # Set ELF_MODE based on configured architectures
      # Support multi-arch setups by checking ARCH environment variable first
      if (defined($ENV{'ARCH'})) {
         if ($ENV{'ARCH'} eq "arm") {
            $makevars{'ELF_MODE'} = "armelf";
         } elsif ($ENV{'ARCH'} eq "x86") {
            if ($sys_kind =~ /FreeBSD/) {
            ...
            my $unsupported_arch = $ENV{'ARCH'};
            print $colors{"YELLOW"}." warning: unsupported ARCH='$unsupported_arch', falling back to default detection".$colors{"STD"};
            undef($ENV{'ARCH'}); # Clear ARCH to trigger fallback logic
         }
      }
