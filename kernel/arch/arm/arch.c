...
uint32_t pok_thread_stack_addr(const uint8_t partition_id,
  ...
  if (stack_addr < partition_base ||
      stack_addr >= (partition_base + partition_size)) {
    return POK_INVALID_STACK_ADDRESS; /* Stack address calculation error */
  }

  /* Validate stack has enough space before partition boundary */
  if (stack_addr < (partition_base + POK_USER_STACK_SIZE)) {
    return POK_INVALID_STACK_ADDRESS; /* Not enough space for full stack */
  }
  ...
  if (aligned_stack_addr < partition_base ||
      aligned_stack_addr >= (partition_base + partition_size)) {
    return POK_INVALID_STACK_ADDRESS; /* Alignment caused address to go out of
                                         bounds */
  }

  /* Post-alignment bounds check: ensure stack guard space is still available
   * For downward-growing stacks, verify guard space below aligned address */
  if (aligned_stack_addr < (partition_base + POK_STACK_GUARD_BYTES)) {
