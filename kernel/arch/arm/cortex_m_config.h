...
static inline pok_ret_t cortex_m_validate_config(void) {
  ...
  extern volatile uint32_t SCB_ICTR; /* Interrupt Controller Type Register */
  ...
  if (CORTEX_M_NVIC_VECTOR_COUNT > hw_total_vectors) {
    printf("WARNING: CORTEX_M_NVIC_VECTOR_COUNT (%u) exceeds hardware "
           "capability (%u)\n",
  ...
  uint32_t hw_mpu_regions = (MPU_TYPE >> 8) & 0xFF;
