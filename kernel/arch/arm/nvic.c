...
pok_ret_t pok_nvic_set_handler(uint8_t irq, void (*handler)(void)) {
  ...
  if (handler == NULL) {
#ifdef POK_NEEDS_DEBUG
    printf("ERROR: NULL handler provided for IRQ %u\n", irq);
#endif
    return POK_ERRNO_EINVAL;
  }
...
pok_ret_t pok_nvic_set_priority(uint8_t irq, uint8_t priority) {
  ...
  if ((priority << 4) & ~ARM_PRIORITY_MASK) {
#ifdef POK_NEEDS_DEBUG
    printf(
        "ERROR: Priority %u not supported by hardware (ARM_PRIORITY_BITS=%u)\n",
        priority, ARM_PRIORITY_BITS);
#endif
    return POK_ERRNO_EINVAL;
  }
