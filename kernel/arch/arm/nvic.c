...
#include "arch.h"
#include "nvic.h"
/* Removed STM32F4-specific includes for generic BSP compilation */
...
static pok_ret_t pok_nvic_relocate_vector_table(void) {
  ...
  if (!in_flash && !in_sram) {
    ...
    printf("ERROR: ROM vector table at invalid address: 0x%x (outside Flash "
           ...
           rom_table_addr, STM32F4_FLASH_BASE,
           STM32F4_FLASH_BASE + STM32F4_FLASH_SIZE - 1, POK_SRAM_BASE,
           POK_SRAM_BASE + POK_SRAM_SIZE - 1);
#endif
  ...
  if (table_end < rom_table_addr || table_end > region_end) {
    ...
    printf("ERROR: ROM vector table (0x%x + %u bytes) extends beyond %s "
           "region (end: 0x%x)\n",
           rom_table_addr, NVIC_VECTOR_TABLE_SIZE, in_flash ? "Flash" : "SRAM",
           region_end);
#endif
    return POK_ERRNO_EINVAL;
  }
...
pok_ret_t pok_nvic_set_priority(uint8_t irq, uint8_t priority) {
  ...
  if (irq >= NVIC_VECTOR_COUNT) {
#ifdef POK_NEEDS_DEBUG
    printf("ERROR: IRQ %u exceeds vector count (%u)\n", irq, NVIC_VECTOR_COUNT);
#endif
    return POK_ERRNO_EINVAL;
  }

  if (priority > NVIC_PRIORITY_LOWEST) {
#ifdef POK_NEEDS_DEBUG
    printf("ERROR: Priority %u exceeds maximum (%u)\n", priority,
           NVIC_PRIORITY_LOWEST);
#endif
    return POK_ERRNO_EINVAL;
  }

  /* Validate priority is within hardware implementation limits */
  uint32_t max_priority = (1U << ARM_PRIORITY_BITS) - 1;
  if (priority > max_priority) {
#ifdef POK_NEEDS_DEBUG
    printf(
        "ERROR: Priority %u exceeds hardware limit %u (ARM_PRIORITY_BITS=%u)\n",
        priority, max_priority, ARM_PRIORITY_BITS);
#endif
    return POK_ERRNO_EINVAL;
  }
...
pok_ret_t pok_nvic_set_vector_table(uint32_t offset) {
  ...
  if (table_end < offset || table_end > region_end) {
    ...
    printf("ERROR: Vector table at 0x%x (size %u bytes) extends beyond %s "
           "region end (0x%x)\n",
           offset, NVIC_VECTOR_TABLE_SIZE, in_flash ? "Flash" : "SRAM",
           region_end);
#endif
    return POK_ERRNO_EFAULT;
  }
