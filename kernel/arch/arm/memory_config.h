...
/* Validate computed user memory size after POK_USER_MEMORY_SIZE definition */
#if (POK_SRAM_SIZE - POK_KERNEL_MEMORY_SIZE) < 0x1000UL
#error                                                                         \
    "Insufficient user memory - need at least 4KB for partitions (SRAM_SIZE - KERNEL_SIZE too small)"
#endif
