...
#define MPU_ATTR_PERIPHERAL_DEVICE                                             \
  (MPU_TEX(2) | MPU_RASR_S) /* TEX=010, C=0, B=0, S=1 - Device memory for      \
                               normal peripherals */
#define MPU_ATTR_PERIPHERAL_DEVICE_XN                                          \
  (MPU_TEX(2) | MPU_RASR_S |                                                   \
   MPU_RASR_XN) /* Device memory + Shareable + Execute Never */
...
#ifndef MPU_MIN_REGION_SIZE
#define MPU_MIN_REGION_SIZE                                                    \
  32 /* ARM Cortex-M minimum MPU region size (2^5 bytes) */
#endif
#define MPU_IS_POWER_OF_2(x) mpu_is_power_of_2((uint32_t)(x))
#define MPU_IS_ALIGNED(addr, size)                                             \
  mpu_is_aligned((uint32_t)(addr), (uint32_t)(size))
#define MPU_IS_VALID_SIZE(x)                                                   \
  ((uint32_t)(x) >= MPU_MIN_REGION_SIZE && mpu_is_power_of_2((uint32_t)(x)))
...
typedef struct {
  ...
  uint8_t region_id;
  uint8_t enabled;
  uint8_t srd_mask; /* SubRegion Disable mask (8 bits, one per subregion) */
} mpu_region_t;
