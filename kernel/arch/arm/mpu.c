...
#ifdef POK_NEEDS_DEBUG
      /* printf calls commented out to avoid linking issues */
...
pok_ret_t pok_mpu_init(void) {
  ...
  if (mpu_region_count == 0) {
    ...
    pok_cons_write("CRITICAL ERROR: No MPU regions detected! MPU_TYPE=", 50);
    /* Print the MPU_TYPE value in hex for debugging */
    char hex_buf[10];
    for (int i = 7; i >= 0; i--) {
      hex_buf[7 - i] = "0123456789ABCDEF"[(mpu_type >> (i * 4)) & 0xF];
    }
    hex_buf[8] = '\n';
    hex_buf[9] = '\0';
...
pok_ret_t pok_mpu_enable_region(uint8_t region) {
  ...
  if (!mpu_regions[region].enabled) {
...
#ifdef POK_NEEDS_DEBUG
    /* Read back configuration for debugging */
    MPU_RNR = region;
    uint32_t rbar = MPU_RBAR;
    uint32_t rasr = MPU_RASR;

    pok_cons_write("MPU enable region ", 18);
    char hex_buf[16];
    hex_buf[0] = '0' + region;
    hex_buf[1] = ':';
    hex_buf[2] = ' ';
    ...
    for (int i = 7; i >= 0; i--) {
      hex_buf[i] = "0123456789ABCDEF"[(rbar >> ((7 - i) * 4)) & 0xF];
    }
    pok_cons_write(hex_buf, 8);

    pok_cons_write(" RASR=0x", 8);
    for (int i = 7; i >= 0; i--) {
      hex_buf[i] = "0123456789ABCDEF"[(rasr >> ((7 - i) * 4)) & 0xF];
    }
    pok_cons_write(hex_buf, 8);
    ...
    if (rasr & (1 << 28)) {
      ...
      pok_cons_write(" [EXEC]", 7);
    }

    uint8_t ap = (rasr >> 24) & 0x7;
    pok_cons_write(" AP=", 4);
    hex_buf[0] = '0' + ap;
    hex_buf[1] = '\n';
    pok_cons_write(hex_buf, 2);
#endif
  }
...
#ifdef POK_NEEDS_DEBUG
  pok_cons_write("MPU ENABLED: MPU_CTRL=0x", 24);
  uint32_t ctrl_val = MPU_CTRL;
  char hex[9];
  for (int i = 7; i >= 0; i--) {
    hex[i] = "0123456789ABCDEF"[(ctrl_val >> ((7 - i) * 4)) & 0xF];
  }
  hex[8] = '\n';
...
#ifdef POK_NEEDS_DEBUG
  pok_cons_write("WARNING: MPU BEING DISABLED!\n", 29);
#endif
