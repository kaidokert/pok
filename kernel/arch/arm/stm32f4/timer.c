...
pok_ret_t pok_timer_init(void) {
  ...
  if (TIMER_RELOAD_VAL < SYSTICK_MIN_RELOAD) {
    ...
    printf("System freq: %u Hz, Tick rate: %u Hz - continuing anyway\n",
           SYSTICK_FREQ_HZ, TIMER_TICK_HZ);
#endif
...
void pok_timer_handler(void) {
  ...
  /* Update POK system time - simple increment sufficient for single-core ARM
   * Cortex-M */
  pok_tick_counter++;
