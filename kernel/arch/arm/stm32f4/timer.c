...
void pok_timer_handler(void) {
  ...
  /* Update POK system time atomically using inline assembly
   * Use load-exclusive/store-exclusive for guaranteed atomicity */
  ...
  do {
    __asm volatile("ldrex %0, [%2]\n\t"
                   ...
                   : "r"(&pok_tick_counter)
                   : "memory");
  } while (old_value != 0); /* Retry if store-exclusive failed */
