...
pok_ret_t pok_cons_init(void) {
  ...
  /* Use 32-bit arithmetic - sufficient for typical STM32F4 clocks and baud
   * rates */
  uint32_t numerator = (apb2_clock * USART_OVERSAMPLING_FACTOR) +
                       (USART_ROUNDING_DIVISOR * baud_rate);
  uint32_t denominator = USART_OVERSAMPLING_FACTOR * baud_rate;
  uint32_t usartdiv_scaled = numerator / denominator;
