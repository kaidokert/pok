...
/* Define UINT32_MAX to avoid conflicts with system stdint.h */
#ifndef UINT32_MAX
...
pok_ret_t pok_cons_init(void) {
  ...
  /* Clear pull-up/pull-down bits for PA9 and PA10 (2 bits per pin) */
  ...
  uint64_t numerator_64 = (uint64_t)apb2_clock * USART_OVERSAMPLING_FACTOR;
  uint64_t denominator_64 = (uint64_t)USART_OVERSAMPLING_FACTOR * baud_rate;
...
pok_bool_t pok_cons_write(const char *s, size_t length) {
  ...
  if (timeout == 0) {
    /* Avoid printf to prevent recursion - don't return error for final flush
