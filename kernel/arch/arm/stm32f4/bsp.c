...
void *pok_bsp_mem_alloc(size_t size) {
  ...
  /* Align to 8-byte boundary: align both size and current pointer */
  size = (size + POK_MEMORY_ALIGNMENT_MASK) & ~POK_MEMORY_ALIGNMENT_MASK;
  uint32_t aligned_addr = (current_alloc_addr + POK_MEMORY_ALIGNMENT_MASK) &
                          ~POK_MEMORY_ALIGNMENT_MASK;
  ...
  if (size > (kernel_end - aligned_addr)) {
#ifdef POK_NEEDS_DEBUG
    printf("ERROR: Kernel memory exhausted. Requested: %u, Available: %u\n",
           size,
           (unsigned int)((POK_KERNEL_MEMORY_BASE + POK_KERNEL_MEMORY_SIZE) -
                          current_alloc_addr));
#endif
    return (NULL);
  }
  ...
  current_alloc_addr = aligned_addr + size;
...
#ifdef POK_NEEDS_DEBUG
  printf("WARNING: pok_bsp_mem_free called but not implemented (ptr=0x%x, "
         "size=%u)\n",
         (uint32_t)ptr, (unsigned)size);
#endif
}
...
pok_ret_t pok_stm32f4_clock_init(void) {
...
#ifdef POK_NEEDS_DEBUG
    printf(
        "WARNING: HSE frequency is %u Hz, but PLL configuration assumes 8MHz\n",
        (unsigned int)HSE_FREQ_HZ);
    printf(
...
#ifdef POK_NEEDS_DEBUG
    printf("WARNING: USB clock will be %u Hz, but USB requires exactly 48MHz\n",
           (unsigned)usb_freq_calculated);
  ...
  *PWR_CR =
  ...
  if (timeout == 0) {
    return POK_ERRNO_EFAULT; /* Voltage regulator failed to stabilize */
  }
