...
pok_ret_t pok_create_space(uint8_t partition_id, uint32_t addr, uint32_t size) {
  ...
  uint32_t initial_waste_percent = (initial_waste * 100) / size;
  ...
  if (initial_waste_percent > MEMORY_WASTE_THRESHOLD_PERCENT &&
    ...
    if (half_aligned >= size && mpu_is_aligned(addr, half_aligned)) {
      ...
      uint32_t subregion_waste_percent = (subregion_waste * 100) / size;
      if (subregion_waste_percent < initial_waste_percent) {
#ifdef POK_NEEDS_DEBUG
        printf("INFO: Using smaller alignment for partition %d: %u->%u (waste: "
               "%u%%->%u%%)\n",
               partition_id, aligned_size, half_aligned, initial_waste_percent,
               subregion_waste_percent);
#endif
...
#ifdef POK_NEEDS_DEBUG
    printf("ERROR: Partition %d base addr 0x%08X not aligned to size %u\n",
  ...
  uint32_t predicted_waste_percent = (predicted_waste * 100) / size;
...
#ifdef POK_NEEDS_DEBUG
      printf("INFO: Using subregions for partition %d: waste %u->%u bytes\n",
             partition_id, current_waste, subregion_waste);
#endif
...
#else
#error "This ARM port is designed for Cortex-M (Thumb-only) processors"
#endif
...
#ifdef POK_NEEDS_DEBUG
    printf("ERROR: Invalid kernel memory layout: base=0x%08X, size=%u\n",
...
#ifdef POK_NEEDS_DEBUG
    printf("ERROR: Kernel size %u is not power-of-2 or below minimum %u\n",
           kernel_size, MPU_MIN_REGION_SIZE);
#endif
    return POK_ERRNO_EINVAL;
  }
...
#ifdef POK_NEEDS_DEBUG
    printf(
        "ERROR: Kernel memory region would overflow: base=0x%08X + size=%u\n",
        kernel_base, kernel_size);
#endif
    return POK_ERRNO_EINVAL;
  }
