...
extern uint8_t
    *pok_ports_destinations[POK_CONFIG_NB_PORTS]; /**  from deployment.c when
                                                     using code generation */
extern uint8_t
    *pok_ports_by_partition[POK_CONFIG_NB_PARTITIONS]; /**  from deployment.c
                                                          when using code
                                                          generation */
extern pok_port_t pok_ports[POK_CONFIG_NB_PORTS]; /**  from deployment.c when
                                                     using code generation */
extern uint8_t
    pok_ports_nodes[POK_CONFIG_NB_GLOBAL_PORTS]; /**  from deployment.c when
                                                    using code generation */
extern uint8_t pok_global_ports_to_local_ports
    [POK_CONFIG_NB_GLOBAL_PORTS]; /**  from deployment.c when using code
                                     generation */
