...
typedef enum {
  ...
  POK_PARTITION_MODE_STOPPED =
      6, /**< Partition is stopped, it will no longer be executed */
 ...
 * \brief This structure contains all needed information for partition
 * management
 */
typedef struct {
  uint32_t base_addr;  /**< The base address inside the whole memory (where the
                          segment is in the whole memory ?) */
  uint32_t base_vaddr; /**< The virtual address of the partition. The address
                        * the threads sees when they are executed
                        */
  ...
  uint16_t io_min; /**< If the partition is allowed to perform I/O, the lower
                      bound of the I/O */
  uint16_t io_max; /**< If the partition is allowed to perform I/O, the uppder
                      bound of the I/O */
#endif
 ...
 * With that, you can do POK_CURRENT_PARTITION.nthreads of
 * POK_CURRENT_PARTITION.mode It avoids tedious syntax like
 * pok_partitions[my_partition].blablabla
 */
